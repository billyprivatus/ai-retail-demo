    ...
    <div className='bg-black'>
      <div className='mx-auto py-10 text-center flex flex-col gap-y-5 items-center justify-center'>
        ...
        <div className='flex gap-3 px-2 mx-auto items-center justify-center flex-wrap'>
          {Links.map((link, idx) => (
            <Link className='hover:text-white text-xs text-[#888888]' key={idx} href={link.href}>
              {link.name}
            </Link>
          ))}
        </div>
      </div>
      <p className='text-center pt-6 text-[#888888] text-xs'>&copy; {year} All Rights Reserved</p>
      <p className='text-center py-2 text-[#888888] text-[9px]'>
        Built by{' '}
        <Link href='/' className='underline italic hover:text-white'>
          RivatLabs
        </Link>
      </p>
    </div>
